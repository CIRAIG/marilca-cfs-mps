...
calculate_ff_statistics <- function(ff_data) {
  ff_stats <- ff_data |>
    ...
    summarise(
      ...
      #ff_cv = ff_sd / ff_mean,
      ff_ll_95 = ff_geom_mean / ff_gsd^1.96, #Computed ul and ll instead of retrieved from the list
...
######
# DYNAMIC FATE functions
######

# Function to calculate mass at specific time points
calculate_mass_over_time <- function(k_matrix_list, setup, time_years = 1:200) {
  
  #Process matrix to get the right k_final
  k_matrix_list <- lapply(k_matrix_list, function(k_mat) {
    # Filter
    k_filtered <- k_mat[setup$keep_idx, setup$keep_idx, drop = FALSE]
  ...
  # Convert time to seconds (to match K_final units)
  seconds_per_year <- 365.25 * 24 * 3600
  time_seconds <- time_years * seconds_per_year
  
  # Get compartment names from setup
  compartment_names <- rownames(setup$A_mat)#[setup$keep_idx]
  
  # Define emission compartments we care about
  emission_compartments <- c("w2R", "w1R", "s1R")
  
  # Find indices of emission compartments in the filtered matrix
  emit_idx <- match(emission_compartments, compartment_names)
  
  # Check if all emission compartments were found
  if(any(is.na(emit_idx))) {
    stop("Some emission compartments not found in filtered matrix")
  }
  
  # Number of Monte Carlo samples
  n_samples <- length(k_matrix_list)
  ...
  for(i in 1:n_samples) {
    ...
    # Get the filtered K_final matrix for this sample
    k_mat <- k_matrix_list[[i]]
    
    # Ensure k_mat has proper names
    if(is.null(rownames(k_mat))) {
      rownames(k_mat) <- compartment_names
      colnames(k_mat) <- compartment_names
    }
    
    # Calculate matrix exponential for all time points
    # Initialize array to store masses: [time, receiving_compartment, emission_compartment]
    n_times <- length(time_seconds)
    n_comps <- length(compartment_names)
    n_emit <- length(emission_compartments)
    
    mass_array <- array(0, dim = c(n_times, n_comps, n_emit),
                        dimnames = list(
                          time = paste0("year_", time_years),
                          receiving = compartment_names,
                          emission = emission_compartments
                        ))
    
    # For each emission compartment
    for(e in 1:n_emit) {
      
      # Create initial condition vector (1 kg in emission compartment e)
      m0 <- rep(0, n_comps)
      m0[emit_idx[e]] <- 1
      
      # Calculate mass at each time point
      for(t in 1:n_times) {
        # Mass at time t: m(t) = exp(K * t) * m0
        exp_Kt <- expm(k_mat * time_seconds[t])
        #print(exp_Kt)
        #print(m0)
        mass_t <- exp_Kt %*% m0
        
        # Store results
        mass_array[t, , e] <- as.vector(mass_t)
      }
    }
    
    # Store this sample's results
    results_list[[i]] <- mass_array
    
    # Progress indicator
    if(i %% 10 == 0) {
      cat("Processed", i, "of", n_samples, "samples\n")
    }
  }
  
  return(list(
    mass_arrays = results_list,
...
# Function to summarize results across Monte Carlo samples
summarize_mass_results <- function(mass_results) {
  
  mass_arrays <- mass_results$mass_arrays
  time_years <- mass_results$time_years
  comp_names <- mass_results$compartment_names
  emit_comps <- mass_results$emission_compartments
  n_samples <- mass_results$n_samples
  
  n_times <- length(time_years)
  n_comps <- length(comp_names)
  n_emit <- length(emit_comps)
  
  # Create summary statistics
  summary_list <- list()
  
  for(e in 1:n_emit) {
    emit_name <- emit_comps[e]
    
    # Extract masses for this emission compartment across all samples
    # Reshape to matrix: [sample, time*compartment]
    mass_matrix <- matrix(0, nrow = n_samples, ncol = n_times * n_comps)
    
    for(i in 1:n_samples) {
      mass_matrix[i, ] <- as.vector(mass_arrays[[i]][, , e])
    }
    
    # Calculate statistics
    mean_mass <- matrix(colMeans(mass_matrix), nrow = n_times, ncol = n_comps)
    median_mass <- matrix(apply(mass_matrix, 2, median), nrow = n_times, ncol = n_comps)
    sd_mass <- matrix(apply(mass_matrix, 2, sd), nrow = n_times, ncol = n_comps)
    
    # Calculate confidence intervals (95%)
    ll_95 <- matrix(apply(mass_matrix, 2, quantile, probs = 0.025), nrow = n_times, ncol = n_comps)
    ul_95 <- matrix(apply(mass_matrix, 2, quantile, probs = 0.975), nrow = n_times, ncol = n_comps)
    
    # Add row/column names
    rownames(mean_mass) <- paste0("year_", time_years)
    colnames(mean_mass) <- comp_names
    
    rownames(median_mass) <- paste0("year_", time_years)
    colnames(median_mass) <- comp_names
    
    rownames(sd_mass) <- paste0("year_", time_years)
    colnames(sd_mass) <- comp_names
    
    rownames(ll_95) <- paste0("year_", time_years)
    colnames(ll_95) <- comp_names
    
    rownames(ul_95) <- paste0("year_", time_years)
    colnames(ul_95) <- comp_names
    
    # Store in list
    summary_list[[emit_name]] <- list(
      mean = mean_mass,
      #median = median_mass,
      #sd = sd_mass,
      ll_95 = ll_95,
...
# Function to save to Excel (one sheet per emission compartment)
save_mass_to_excel <- function(summary_results, filename = "mass_over_time.xlsx") {
  ...
  # For each emission compartment, create a sheet
  for(emit_name in names(summary_results)) {
    
    # Create a sheet
    addWorksheet(wb, sheetName = emit_name)
    
    # Get the summary for this emission compartment
    emit_summary <- summary_results[[emit_name]]
    
    # Prepare data for Excel
    # We'll put time in first column, then each compartment's mean, ll, ul
    n_times <- nrow(emit_summary$mean)
    n_comps <- ncol(emit_summary$mean)
    
    # Create header
    header <- c("Year")
    for(comp in colnames(emit_summary$mean)) {
      header <- c(header, 
                  paste0(comp, "_mean"),
                  paste0(comp, "_ll_95"),
                  paste0(comp, "_ul_95"))
    ...
    # Create data matrix
    data_matrix <- matrix(NA, nrow = n_times, ncol = length(header))
    colnames(data_matrix) <- header
    
    # Fill in years
    data_matrix[, "Year"] <- as.numeric(gsub("year_", "", rownames(emit_summary$mean)))
    
    # Fill in data for each compartment
    col_idx <- 2
    for(j in 1:n_comps) {
      comp_name <- colnames(emit_summary$mean)[j]
      
      data_matrix[, col_idx] <- emit_summary$mean[, j]
      data_matrix[, col_idx + 1] <- emit_summary$ll_95[, j]
      data_matrix[, col_idx + 2] <- emit_summary$ul_95[, j]
      
      col_idx <- col_idx + 3
    }
    
    # Write to sheet
    writeData(wb, sheet = emit_name, x = data_matrix, rowNames = FALSE)
  }
...
########
#Dynamic Impact: CFs over time
#######


# Function to calculate cumulative CF (PDF·m²·yr) over time
calculate_cumulative_cf_over_time <- function(mass_results, eef_samples, 
                                              states, compartment_names,
                                              volume_compartments, areas_compartments,
                                              SDF, SF, list_tot_species,
                                              FracSpe_wc_aqua, FracSpe_sed_aqua,
                                              FracSpe_ws_marine, FracSpe_wc_marine, FracSpe_sed_marine,
  ...
  mass_arrays <- mass_results$mass_arrays
  time_years <- mass_results$time_years
  comp_names <- mass_results$compartment_names        # e.g. "aR", "w1R", ... (without "S")
  emit_comps <- mass_results$emission_compartments    # e.g. "w2R", "w1R", "s1C"
  n_samples <- length(mass_arrays)
  n_times <- length(time_years)
  n_comps <- length(comp_names)
  n_emit <- length(emit_comps)
  
  # Prepare storage: for each emission compartment, a matrix [sample, time] of CF values
  cf_results <- list()
  for (e in 1:n_emit) {
    cf_results[[emit_comps[e]]] <- matrix(NA, nrow = n_samples, ncol = n_times)
  }
    ...
    for (e in 1:n_emit) {
      ...
      for (t in 1:n_times) {
        ...
        Masses_df <- data.frame(
          Abbr = paste0(comp_names, "S"),   # match states Abbr (e.g. "aRS")
          FF = integrated_masses,
          stringsAsFactors = FALSE
        )
        
        # Process single iteration (reuse your existing function)
        iter_data <- process_single_iteration(
          Masses_df = Masses_df,
          eef_iter = eef_iter,
          states = states,
          ...
          size = size,
          shape = shape,
          emission_compartment_name = compartment_names[emit_comps[e]],
          volume_compartments = volume_compartments,
          areas_compartments = areas_compartments
        )
        
        # Calculate PDF·m²·yr for this iteration
        cf_pdf_m2 <- calculate_pdf_m2_year(iter_data, SF,
                                           FracSpe_wc_aqua, FracSpe_sed_aqua,
                                           FracSpe_ws_marine, FracSpe_wc_marine, FracSpe_sed_marine)
        
        # Store total CF (PDF·m²·yr)
        cf_results[[emit_comps[e]]][i, t] <- cf_pdf_m2$CF_end_PDF_m2_year
      }
    }
    
    # Progress indicator
    if (i %% 10 == 0) cat("Processed CF for sample", i, "\n")
  }
  
  # Summarize across samples for each emission compartment
  summary_list <- list()
  for (e in 1:n_emit) {
    emit_name <- emit_comps[e]
    cf_matrix <- cf_results[[emit_name]]  # [sample, time]
    
    # Compute geomean, 95% CI (assuming lognormal)
    geomean <- apply(cf_matrix, 2, function(x) exp(mean(log(x), na.rm = TRUE)))
    ll_95 <- apply(cf_matrix, 2, quantile, probs = 0.025, na.rm = TRUE)
    ul_95 <- apply(cf_matrix, 2, quantile, probs = 0.975, na.rm = TRUE)
    
    summary_list[[emit_name]] <- data.frame(
      Year = time_years,
      geomean = geomean,
      ll_95 = ll_95,
      ul_95 = ul_95
...
summary_results <- summarize_mass_results(mass_results)


# After you have mass_results and summary_results (from mass calculation)
cat("Calculating cumulative CF over time...\n")

cf_cumulative <- calculate_cumulative_cf_over_time(
...
for (emit_name in names(summary_results)) {
  ...
  # Get mass data for this compartment
  mass_data <- summary_results[[emit_name]]
  n_times <- nrow(mass_data$mean)
  comp_names <- colnames(mass_data$mean)
  
  # Build data frame with mass columns
  df <- data.frame(Year = as.numeric(gsub("year_", "", rownames(mass_data$mean))))
  
  # Add mass columns (mean, ll, ul for each compartment)
  for (comp in comp_names) {
    df[[paste0(comp, "_mean")]] <- mass_data$mean[, comp]
    df[[paste0(comp, "_ll_95")]] <- mass_data$ll_95[, comp]
    df[[paste0(comp, "_ul_95")]] <- mass_data$ul_95[, comp]
  }
  
  # Add cumulative CF columns from cf_cumulative for this emission compartment
  cf_data <- cf_cumulative[[emit_name]]
  df$CF_geomean <- cf_data$geomean
  df$CF_ll_95 <- cf_data$ll_95
  df$CF_ul_95 <- cf_data$ul_95
  
  # Replace the sheet
  if (emit_name %in% names(wb)) {
    openxlsx::removeWorksheet(wb, emit_name)
  }
  openxlsx::addWorksheet(wb, emit_name)
  openxlsx::writeData(wb, emit_name, df, withFilter = FALSE)
}
